...
- (id)init {
    if ((self = [super initWithWindowNibName:@"ELInspector"])) {
        ...
        oscillatorEditors      = [NSMapTable strongToStrongObjectsMapTable];
    }
...
- (void)windowBecameMain:(NSNotification *)notification {
    ...
    if ([windowController respondsToSelector:@selector(layer)]) {
        ELPlayer *player = [(ELDial *)[windowController layer] player];
...
- (void)playerSelected:(ELPlayer *)player_ {
    _player = player_;
